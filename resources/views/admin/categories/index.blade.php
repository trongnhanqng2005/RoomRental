...
@section('content')
    <section class="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
        <header class="mb-7">
            ...
            <h1 class="text-3xl font-bold tracking-[-0.04em] text-slate-950">{{ __('ui.catalogs.categories_heading') }}</h1>
            <p class="mt-3 max-w-3xl text-sm leading-7 text-slate-600">{{ __('ui.catalogs.categories_description') }}</p>
        </header>
        ...
        <form class="mb-8 space-y-4 rounded-panel border border-line bg-white p-4 sm:p-6" method="POST" action="{{ route('admin.categories.store') }}">
            @csrf
            <h2 class="text-lg font-bold text-slate-950">{{ __('ui.catalogs.create_heading') }}</h2>
            <x-ui.error-summary :fields="['name' => __('ui.catalogs.name'), 'description' => __('ui.catalogs.description')]" id="category-create-errors" />
            ...
            @forelse ($categories as $category)
                <article class="rounded-panel border border-line bg-white p-4 sm:p-5">
                    <div class="flex flex-wrap items-start justify-between gap-3">
                        <div class="min-w-0">
                            <h2 class="break-words text-lg font-bold text-slate-950">{{ $category->name }}</h2>
                            @if ($category->description)
                                <p class="mt-1 break-words text-sm leading-6 text-slate-600">{{ $category->description }}</p>
                            @endif
                        </div>
                        <span class="rounded-full px-2.5 py-1 text-xs font-bold {{ $category->is_active ? 'bg-success-50 text-success-700' : 'bg-slate-100 text-slate-700' }}">{{ $category->is_active ? __('ui.catalogs.active') : __('ui.catalogs.hidden') }}</span>
                    </div>
                    <p class="mt-3 text-sm text-slate-600">{{ __('ui.catalogs.usage_count') }}: <span class="font-bold text-slate-900">{{ $category->listings_count }}</span></p>
                    <div class="mt-4 flex flex-wrap gap-2">
                        <a class="inline-flex min-h-11 items-center justify-center rounded-control border border-line px-4 text-sm font-bold text-slate-700 hover:bg-slate-50 focus-visible:outline-2 focus-visible:outline-brand-600" href="{{ route('admin.categories.edit', $category) }}">{{ __('ui.catalogs.edit') }}</a>
                        @if ($category->is_active)
                            <form method="POST" action="{{ route('admin.categories.hide', $category) }}" onsubmit="return confirm(@js(__('ui.catalogs.hide_confirmation')))" data-submit-once>
                                @csrf
                                @method('PATCH')
                        ...
                        @else
                            <form method="POST" action="{{ route('admin.categories.restore', $category) }}" onsubmit="return confirm(@js(__('ui.catalogs.restore_confirmation')))" data-submit-once>
                                @csrf
                                @method('PATCH')
